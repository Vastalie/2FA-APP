...
app.post('/login', (req, res) => {
  ...
  db.query(query, [username, password], (err, results) => {
    ...
    const updateQuery = 'UPDATE users SET secret = ? WHERE id = ?';
    db.query(updateQuery, [newSecret, userId], (err) => {
      if (err) {
        console.error('Failed to update secret:', err);
        return res.status(500).send('Failed to update secret');
      }
      ...
      const uniqueAccountName = `${username}-${Date.now()}-${randomSuffix}`;

      // Create a unique issuer
      const uniqueIssuer = `My2FAApp-${Date.now()}-${randomSuffix}`;

      // Generate a unique otpauth URL
      const otpauth = otplib.authenticator.keyuri(uniqueAccountName, uniqueIssuer, newSecret);
